...
let EventsService = class EventsService {
    ...
    async create(createEventDto) {
        ...
        const tiers = ticketTiers && ticketTiers.length > 0
            ? ticketTiers
    ...
    async ensureVenueSeating(venueId) {
    ...
    async getSeatMap(eventId) {
        ...
        const tierForRow = (rowName) => {
            ...
            if (rowIndex <= 4)
                return tiers.length > 2 ? tiers[1] : tiers[tiers.length - 1];
            return tiers[0];
        };
        const seats = sections.flatMap((section) => section.seats.map((seat) => {
