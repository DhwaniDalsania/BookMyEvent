...
async function main() {
    ...
    for (const v of venueData) {
        const venue = await prisma.venue.create({ data: v }).catch(async () => {
            return prisma.venue.findFirst({ where: { name: v.name, city: v.city } });
        });
    ...
    const getVenueId = (index) => venueIds[index % venueIds.length];
    const now = new Date();
    ...
    for (const e of eventsData) {
        ...
        if (!catId) {
            console.warn(`⚠️ Category not found: ${e.category}`);
            continue;
        }
        const venueId = venueIds[e.venue];
        if (!venueId) {
            console.warn(`⚠️ Venue not found at index: ${e.venue}`);
            continue;
        }
